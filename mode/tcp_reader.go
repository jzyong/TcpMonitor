...
// 复制于tcpassembly/tcpreader/reader.go
// 暴露Current 获取Assembly，为了获取消息包的截取时间
...
// GetCurrentTime 获取当前消息包的截取时间，如果没有数据，返回当前时间
func (r *ReaderStream) GetCurrentTime() time.Time {
	r.stripEmpty()
	if len(r.current) > 0 {
		current := &r.current[0]
		return current.Seen
